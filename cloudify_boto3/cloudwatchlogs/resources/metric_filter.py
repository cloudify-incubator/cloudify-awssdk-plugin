...
"""
    ...
    ~~~~~~~~~~~~~~
    AWS Cloudwatch Logs Metric Filter interface
"""
# Cloudify
...
RESOURCE_TYPE = 'Cloudwatch Logs Metric Filter'
RESOURCE_NAME = 'filterName'
SEARCH_KEY = 'filterNamePrefix'
PARENT_RESOURCE_NAME = 'logGroupName'
RESOURCES = 'metricFilters'


class CloudwatchMetricFilter(AWSCloudwatchLogsBase):
    """
        AWS Cloudwatch Metric Filter interface
    """
    ...
    def properties(self):
        ...
        try:
            resources = \
                self.client.describe_metric_filters(**params)
        except ClientError:
            pass
        else:
    ...
    def create(self, params):
        """
            Create a new AWS Cloudwatch Metric Filter.
        """
        self.logger.debug('Creating %s with parameters: %s'
                          % (self.type_name, params))
        res = self.client.put_metric_filter(**params)
        self.logger.debug('Response: %s' % res)
        return res

    def delete(self, params=None):
        """
            Deletes an existing AWS Cloudwatch Metric Filter.
        """
        self.logger.debug('Deleting %s with parameters: %s'
                          % (self.type_name, params))
        res = self.client.delete_metric_filter(**params)
        self.logger.debug('Response: %s' % res)
        return res
...
def prepare(ctx, resource_config, **_):
    """Prepares an AWS Cloudwatch Metric Filter"""
    # Save the parameters
    ctx.instance.runtime_properties['resource_config'] = resource_config


@decorators.aws_resource(CloudwatchMetricFilter, RESOURCE_TYPE)
def create(ctx, iface, resource_config, **_):
    """Creates an AWS Cloudwatch Metric Filter"""
...
def delete(iface, ctx, resource_config, **_):
    """Deletes an AWS Cloudwatch Metric Filter"""
    # Create a copy of the resource config for clean manipulation.
    params = \
        dict() if not resource_config else resource_config.copy()
    parent_name = params.get(PARENT_RESOURCE_NAME)
