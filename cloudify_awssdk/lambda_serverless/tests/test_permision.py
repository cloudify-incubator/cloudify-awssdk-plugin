...
from cloudify_awssdk.lambda_serverless.resources import permission
import unittest
from cloudify.manager import DirtyTrackingDict
from cloudify_awssdk.common.tests.test_base import TestBase, mock_decorator

PATCH_PREFIX = 'cloudify_awssdk.lambda_serverless.resources.permission.'
...
class TestLambdaPermission(TestBase):
    ...
    def setUp(self):
        ...
        mock1 = patch('cloudify_awssdk.common.decorators.aws_resource',
                      mock_decorator)
        mock2 = patch('cloudify_awssdk.common.decorators.aws_relationship',
                      mock_decorator)
        mock1.start()
        mock2.start()
        reload(permission)
    ...
    def test_create(self):
        ...
        with patch(PATCH_PREFIX + 'utils') as utils:
            iface = MagicMock()
            iface.create = MagicMock(return_value=('res_id', 'res_arn'))
        ...
        with patch(PATCH_PREFIX + 'utils') as utils:
            ...
            ctx.instance.runtime_properties['resource_config'].update(
                {'StatementId': 'test_id'})
            permission.create(ctx, iface, {})
            self.assertEqual(2, utils.update_resource_id.call_count)
            self.assertEqual(1, utils.update_resource_arn.call_count)
